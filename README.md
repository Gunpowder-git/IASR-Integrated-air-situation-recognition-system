...
IASR 是一个可扩展的低空态势识别与预警集成系统（MVP）。系统以**视频**为输入，进行**目标检测+跟踪**，计算**交通态势指标**（流量/占有率/速度）与**拥堵指数**，同时给出基于视频清晰度的**能见度态势估计**（雾/霾/烟的代理指标），最终输出统一格式的**事件（Event）**，并根据配置将事件路由给不同对象（运行、安防、应急、交管等）。
