<font color="gray">IASR System 是一个可扩展的低空态势识别与预警集成系统。系统以视频为输入，进行目标检测+跟踪，计算交通态势指标（流量/占有率/速度）与拥堵指数，同时给出基于视频清晰度的能见度态势估计（雾/霾/烟的代理指标），最终输出统一格式的事件，并根据配置将事件路由给运行、安防、应急、交管等对象。项目基于 Streamlit 仪表盘开发，通过 YOLO 实现视频目标检测与跟踪，Opencv 实现静态图片识别。<b>该项目获同济大学国豪书院第三届“研学济豪”海报展三等奖。</b></font>
